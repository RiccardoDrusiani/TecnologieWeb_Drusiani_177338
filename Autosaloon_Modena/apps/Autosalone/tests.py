...
class FlussoCompletoViewTest(TestCase):
    ...
    def test_flusso_completo_view(self):
        ...
        response = self.client.post(f'/Auto/{auto2_pk}/contrattazione/', {'prezzo_attuale': 100 - i, 'stato': 1})
        self.assertIn(response.status_code, [200, 302], f"Contrattazione auto2 step {i} fallita")
        ...
        self.client.post('/Utente/logout/')
        for i in range(5):
            response = self.client.post('/Utente/login/', {'username': 'utente2', 'password': 'pass2'})
